...
import * as math_utils from './math';
...
export const hexFromInt = (argb: number) => {
	...
	const outParts = [tostring(r), tostring(g), tostring(b)];

	// Pad single-digit output values
	for (const [i, part] of outParts) {
		if (part.size() === 1) {
			outParts[<any>i] = '0' + part;
		}
	}
...
export const intFromRgb = (rgb: number[]) => {
	return ((255 << 24) | ((rgb[0] & 0x0ff) << 16) | ((rgb[1] & 0x0ff) << 8) | (rgb[2] & 0x0ff)) >>> 0;
};
...
export const intFromXyzComponents = (x: number, y: number, z: number) => {
	...
	const rInt = math.round(math_utils.clamp(0, 255, r * 255));
	const gInt = math.round(math_utils.clamp(0, 255, g * 255));
	const bInt = math.round(math_utils.clamp(0, 255, b * 255));
...
function parseIntHex(value: string) {
	...
	return <number>tonumber(value, 16);
}
...
export const intFromLstar = (lstar: number) => {
	...
	const xyz = [x * WHITE_POINT_D65[0], y * WHITE_POINT_D65[1], z * WHITE_POINT_D65[2]];
