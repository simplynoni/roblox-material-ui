...
export class HCT {
	...
	private constructor(public internalHue: number, public internalChroma: number, public internalTone: number) {
...
function getInt(hue: number, chroma: number, tone: number): number {
	return getIntInViewingConditions(
		math_utils.sanitizeDegrees(hue),
		chroma,
		math_utils.clamp(0.0, 100.0, tone),
...
function getIntInViewingConditions(
	...
	hue = math_utils.sanitizeDegrees(hue);
 ...
 *     or undefined.
 */
function findCamByJ(hue: number, chroma: number, tone: number): CAM16 | undefined {
	let low = 0.0;
	let high = 100.0;
	let mid = 0.0;
	let bestdL = 1000.0;
